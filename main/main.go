...
func getArgsV4Compatible() []string {
	...
	if len(os.Args) == 1 {
		fmt.Println("命令行仅有程序xray，设置默认command为run，返回xray run")
		return []string{os.Args[0], "run"}
	}
	// os.Args[1]不以-开头，表示xray后指定了command，例如xray uuid，直接返回
	if os.Args[1][0] != '-' {
		fmt.Println("已为程序xray指定了command，直接返回")
		return os.Args
	}
	version := false
	...
	if err == flag.ErrHelp {
		...
		// fmt.Println()
		fmt.Println("检测到ErrHelp错误：解析到-h或者-help参数，但是未定义该flag，返回xray help")
	...
	if version {
		...
		// fmt.Println()
		fmt.Println("解析到-version or --version or -version=true，返回xray version")
	...
	// 1. xray -run -h
	// 2. xray -run -version
	fmt.Println("命令行参数格式错误，设置默认command为run，返回其他命令行参数")
